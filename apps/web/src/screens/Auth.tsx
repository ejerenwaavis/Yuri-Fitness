  ...
  return (
    <div className="min-h-screen bg-background flex flex-col justify-center items-center p-4">
      <div className="w-full max-w-sm bg-surface p-8 rounded-lg border border-surfaceElevated shadow-2xl">
        ...
        <h2 className="text-xl font-bold text-textPrimary mb-4">{t('auth.welcome')}</h2>
        ...
        <div className="relative flex items-center py-2 mb-4">
          ...
          <span className="flex-shrink-0 mx-4 text-textMuted text-sm">{t('auth.or')}</span>
          <div className="flex-grow border-t border-surfaceElevated"></div>
        </div>

        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          <div>
            <label className="block text-sm text-textMuted mb-1">{t('auth.email')}</label>
            <input 
              type="email" 
          ...
          <div>
            <label className="block text-sm text-textMuted mb-1">{t('auth.password')}</label>
            <input 
              type="password" 
              className="w-full bg-background border border-surfaceElevated rounded-md px-4 py-2 text-textPrimary focus:outline-none focus:border-primary"
              placeholder="••••••••"

...
export const substituteExercise = async (
  ...
  const filtered = candidates.filter(c => {
    ...
    for (const inj of userInjuries) {
      ...
      if ((injStr.includes('shoulder') || injStr.includes('rotator')) && (c.substitutionTags.includes('vertical_press') || cName.includes('overhead'))) return false;
      if ((injStr.includes('knee') || injStr.includes('patellar')) && (c.substitutionTags.includes('knee_flexion') || cName.includes('lunge') || cName.includes('jump'))) return false;
      if ((injStr.includes('lower_back') || injStr.includes('back') || injStr.includes('spine')) && (cName.includes('deadlift') || c.substitutionTags.includes('barbell_back'))) return false;
      if (injStr.includes('wrist') && (cName.includes('pushup') || cName.includes('clean'))) return false;
      if (injStr.includes('elbow') && (cName.includes('skull crusher') || cName.includes('dip'))) return false;
      if (injStr.includes('neck') && (cName.includes('behind') || cName.includes('shrug'))) return false;
      
      const words = injStr.split(/\s+/).filter(w => w.length > 3 && !['pain', 'hurt', 'injury', 'left', 'right'].includes(w));
      for (const word of words) {
        if (cName.includes(word) || c.muscleGroups.some(m => m.toLowerCase().includes(word))) return false;
      }
    }
    return true;
  });
